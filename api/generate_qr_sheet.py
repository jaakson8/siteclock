...
def draw_code(canvas, payload, action, x, y, width, height, accent):
    ...
    instruction = "SKANEERI SISENEMISEL" if action == "IN" else "SKANEERI LAHKUMISEL"
...
def make_pdf(data, output):
    ...
    canvas.drawString(34 * mm, page_height - 25 * mm, f"Sissepääs: {data['gateName']}")
    margin = 15 * mm
    gap = 7 * mm
    card_width = (page_width - 2 * margin - gap) / 2
    card_height = 122 * mm
    y = page_height - 158 * mm
    draw_code(canvas, data["inQrBase64"], "IN", margin, y, card_width, card_height, colors.HexColor("#176B48"))
    draw_code(canvas, data["outQrBase64"], "OUT", margin + card_width + gap, y, card_width, card_height, colors.HexColor("#A63D25"))
    canvas.setFillColor(colors.HexColor("#172126"))
    canvas.setFont("Helvetica-Bold", 13)
    canvas.drawCentredString(page_width / 2, y - 14 * mm, "Ava SiteClocki äpp ja skaneeri õige kood")
    canvas.setFont("Helvetica", 9)
    canvas.setFillColor(colors.HexColor("#64737B"))
    canvas.drawCentredString(page_width / 2, y - 21 * mm, "Asukohta kontrollitakse ainult registreerimise hetkel.")
    canvas.setStrokeColor(colors.HexColor("#D9E0E3"))
    canvas.line(margin, 18 * mm, page_width - margin, 18 * mm)
    canvas.setFont("Helvetica", 8)
    canvas.drawString(margin, 12 * mm, "SiteClock - töömaa kohaloleku registreerimine")
